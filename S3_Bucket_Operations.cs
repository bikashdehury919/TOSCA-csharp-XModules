    ...
    class S3_Bucket_Operations_Engine : SpecialExecutionTask
    {
        ...
        public override ActionResult Execute(ISpecialExecutionTaskTestAction testAction)
        {
            try
            {
                ...
                if (Ioperation == null)
                    return new UnknownFailedActionResult("Operation parameter is missing.");
                ...
                try
                {
                    ...
                    var buckets = s3Client.ListBucketsAsync().Result; // Force credential check
                    Log("AWS credentials validated successfully.");
                }
                catch (AmazonS3Exception ex)
                {
                    Log($"Invalid AWS credentials: {ex.Message}");
                    return new UnknownFailedActionResult($"Invalid AWS credentials: {ex.Message}\n" + GetLogs());
                }
                catch (Exception ex)
                {
                    Log($"Unexpected error during AWS credentials check: {ex.Message}");
                    return new UnknownFailedActionResult($"Unexpected error during AWS credentials check: {ex.Message}\n" + GetLogs());
                }
                ...
                {
                    case "Upload File":
                        return UploadFileAsync(s3Client, bucketName, testAction).Result;
                    case "Download File":
                        return DownloadFileAsync(s3Client, bucketName, testAction).Result;
                    case "List Files":
                        return ListFilesAsync(s3Client, bucketName, testAction).Result;
                    case "Delete File":
                        return DeleteFileAsync(s3Client, bucketName, testAction).Result;
                    case "Check File Exists":
                        return FileExistsAsync(s3Client, bucketName, testAction).Result;
					case "Download Dat File":
						return DownloadMostRecentDatFileAsync(s3Client, bucketName, testAction).Result;
                    default:
                        Log("Invalid operation specified.");
                        return new UnknownFailedActionResult("Invalid operation specified.");
                }
            }
            catch (Exception ex)
            {
		...
		private async Task<ActionResult> DownloadMostRecentDatFileAsync(IAmazonS3 s3Client, string bucketName, ISpecialExecutionTaskTestAction testAction)
		{
			...
			try
			{
				...
				if (Is3_FilePath == null)
					return new UnknownFailedActionResult("S3_FilePath parameter is mandatory for Download Most Recent Dat File operation.");
				if (IlocalFilePath == null)
					return new UnknownFailedActionResult("LocalFilePath parameter is mandatory for Download Most Recent Dat File operation.");

				string s3FilePath = Is3_FilePath.Value.Trim();
				string localFolderPath = IlocalFilePath.Value.Trim();

				Log($"Listing .dat files in S3 bucket: {bucketName} with prefix: {s3FilePath}");

				// List objects with prefix
				var request = new ListObjectsRequest
				{
					BucketName = bucketName,
					Prefix = s3FilePath.EndsWith("/") ? s3FilePath : s3FilePath + "/"
				};

				var response = await s3Client.ListObjectsAsync(request);

				// Find the most recent .dat file by LastModified
				var datFiles = response.S3Objects
					.Where(o => o.Key.EndsWith(".dat", StringComparison.OrdinalIgnoreCase))
					.OrderByDescending(o => o.LastModified)
					.ToList();

				if (datFiles.Count == 0)
				{
					Log("No .dat files found in the specified S3 path.");
					return new PassedActionResult($"No .dat files found in s3://{bucketName}/{s3FilePath}\n" + GetLogs());
				}

				var mostRecentDatFile = datFiles.First();
				s3Key = mostRecentDatFile.Key;

				Log($"Most recent .dat file found: {s3Key}");

				// Prepare local download path (use file name from S3 key)
				string fileName = Path.GetFileName(s3Key);
				localFilePath = Path.Combine(localFolderPath, fileName);

				Log($"Starting download of {s3Key} to local path: {localFilePath}");

				var transferUtility = new TransferUtility(s3Client);
				await transferUtility.DownloadAsync(localFilePath, bucketName, s3Key);

				Log("Download completed successfully.");

				return new PassedActionResult($"Most recent .dat file downloaded successfully: {localFilePath}\n" + GetLogs());
			}
			catch (AmazonS3Exception ex)
			{
				Log($"AWS S3 error during download of most recent .dat file: {ex.Message}");
				return new UnknownFailedActionResult($"AWS S3 error during download of most recent .dat file: {ex.Message}\n" + GetLogs());
			}
			catch (Exception ex)
			{
				Log($"General error during download of most recent .dat file: {ex.Message}");
				return new UnknownFailedActionResult($"General error during download of most recent .dat file: {ex.Message}\n" + GetLogs());
			}
		}
